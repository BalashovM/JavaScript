...
export class Good{
    ...
    #pictureQuantity//Количество изображений товара

    constructor(type, name, price, unit, picturePrefix, pictureQuantity) {
        this.#id = new Guid();
        this.#type = type;
        this.#name = name;
        this.#price = price;
...
export class Storage{
    ...
    //Получение товара по названию изображения
    getGoodByPicturePrefix(picturePrefix){
        let found = null;
        for (const item of this.#goods) {
            let good = item.getGood();
            if (good.getPicturePrefix() === picturePrefix) {
                found = good;
            }
        }
        return found;   
...
export class Cart extends Storage{
    ...
    render() {
        ...
        var prevCartAmount =  this.countAmount();
        if (prevCartAmount != 0) {
            ...
            let buttonNext = document.getElementById('next_button');
